...
const char *test_wsum_hess_quad_form_dense_param(void)
{
    ...
    expr *x = new_variable(3, 1, 2, 5);
    expr *node = new_quad_form_dense(x, 3, NULL, param_P); /* P fed by parameter */
    ...
    mu_assert("P1 value fail", fabs(node->value[0] - 57.0) < 1e-9);
    ...
    /* update the parameter; the refresh recomputes only the values */
    ...
    mu_assert("P2 value fail", fabs(node->value[0] - 67.0) < 1e-9);
    mu_assert("P2 grad vals fail", cmp_values(node->jacobian, grad2, 3));
    mu_assert("P2 hessian vals fail", cmp_values(node->wsum_hess, hess2, 9));
